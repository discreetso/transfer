...
; Assemble: 
;   nasm -f elf32 -l coal12.lst -o coal12.o coal12.asm
...
;       ld -m elf_i386 -o coal12 coal12.o
...
; a section 'directive' marks the part of a program 
; for the ELF format (or whatever binary format you are using)

SECTION .text

; we mark the start of this library using the following:
GLOBAL _start
...
_start:
    ...
    mov ebx, 0                      ; exit code is 0 (like c/c++, return 0)
    int 0x80                        ; comment out and see
; note that int is NOT the right way to do things! (more on this later)
